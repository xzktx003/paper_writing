      ...
      <div className={styles.content}>
        ...
        {stage === 'produce' && <>
          <section className={styles.section} aria-labelledby="office-artifact-title">
            ...
            {artifactResult && <pre className={styles.resultCode} data-testid="office-artifact-result">{JSON.stringify(artifactResult, null, 2)}</pre>}
          </section>
          ...
          <section className={styles.section} aria-labelledby="office-meeting-title">
            ...
            {snapshot.workspace.meetings[0] && <div className={styles.meetingResult}><strong>{snapshot.workspace.meetings[0].title}</strong><p>{snapshot.workspace.meetings[0].summary}</p><span>{language === 'zh' ? '决定' : 'Decisions'} {snapshot.workspace.meetings[0].decisions.length} · {language === 'zh' ? '待办' : 'Actions'} {snapshot.workspace.meetings[0].actionItems.length} · diarization {snapshot.workspace.meetings[0].source.speakerDiarization}</span></div>}
          </section>
        </>}
        ...
        {stage === 'measure' && <>
          <section className={styles.section} aria-labelledby="office-measure-title">
            ...
            <button className={`${styles.button} ${styles.buttonPrimary}`} data-testid="office-record-metric" type="button" disabled={busy != null} onClick={() => void recordMetric()}>{language === 'zh' ? '记录本次真实样本' : 'Record this real sample'}</button>
            {efficiency && <div className={styles.efficiencyResult} data-testid="office-efficiency-summary"><span className={`${styles.stateLabel} ${efficiency.evidenceStatus === 'sufficient' ? styles.stateReady : styles.stateBlocked}`}>{efficiency.evidenceStatus}</span>{efficiency.evidenceStatus === 'sufficient' ? <><div><strong>{efficiency.metrics.timeSavedMinutes ?? '—'}</strong><span>{language === 'zh' ? '平均净节省分钟' : 'average net minutes saved'}</span></div><p>{language === 'zh' ? '采纳率' : 'Acceptance'} {efficiency.metrics.acceptanceRate == null ? '—' : `${Math.round(efficiency.metrics.acceptanceRate * 100)}%`} · {language === 'zh' ? '样本' : 'Samples'} {efficiency.coverage.sampleSize}</p></> : <p>{efficiency.insufficiencyReasons.join('；')}</p>}</div>}
          </section>
