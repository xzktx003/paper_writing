...
Low-bit weight quantization reduces the storage and weight-loading cost of large language models, and its effectiveness depends on how well weights and model responses are preserved under a limited representation budget~\cite{gptq,awq,omniquant,spqr,squeezellm}.
% 向量量化（VQ）用一个索引联合表示多个参数，相比逐标量编码具有更灵活的量化几何。
Vector quantization (VQ) jointly represents multiple parameters with a single index and therefore offers a more flexible quantization geometry than scalar encoding~\cite{grayneuhoff,gershogray}.
...
% 附录 A 给出一个可核算的存在性实例：两个单位置更新在各自重调尺度后都变差，而同步修改两个位置可以消除误差。
Appendix~\ref{app:example} gives a checkable existence instance in which both one-at-a-time updates become worse after their respective scale refits, whereas changing two positions together eliminates the error.
% 图 1 进一步用两阶段轨迹直观展示联合停滞：单位置索引更新与尺度重优化收敛后，误差在切换点附近形成平台；改用联合索引更新后，三个尺度组的真实层输出误差继续下降。
Figure~\ref{fig:motivation} further visualizes joint stagnation with a two-stage trajectory. After single-index updates with scale re-optimization converge, the error forms a plateau around the switching point; after switching to joint-index updates, the true layer-output error continues to decrease for all three scale groups.
% 右图给出 116 次被接受更新中改动索引数的分布，其中 97.4\% 的更新同时改变至少两个索引，说明有效更新主要来自协调的多位置变化。
The right panel reports the number of changed indices across 116 accepted updates. In total, 97.4\% of the accepted updates change at least two indices, indicating that the effective moves are predominantly coordinated multi-position changes.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figure1_motivation.png}
\caption{
% 共享尺度下的联合停滞与多索引更新。（a）三个共享尺度组在单位置索引更新与尺度重优化阶段逐步进入平台；切换到联合索引更新后，归一化真实层输出误差继续下降。每条曲线以切换时的误差归一化为 1。（b）被接受更新的索引改动数分布；多索引更新占多数（$h\geq2$：97.4\%，$N=116$）。
Joint stagnation and multi-index updates under shared scales. (a) Three shared-scale groups approach a plateau under single-index updates with scale re-optimization; after switching to joint-index updates, normalized true layer-output error resumes its descent. Each trajectory is normalized to 1 at the switch. (b) Distribution of the number of changed indices per accepted update; multi-index updates predominate ($h\geq2$: 97.4\%, $N=116$).
}
...
The procedure directly updates hard indices and the stored scale without introducing probabilistic index parameters or backpropagation; its optimality is restricted to the surrogate objective over the current candidate pool.
