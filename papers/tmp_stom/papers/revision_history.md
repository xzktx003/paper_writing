...
| V15 | 2026-08-31 | Hard-first touched-group闭式scale补偿几乎一致降低assignment文本代价，却仍无法打开零退化可行域 | 26/27可比动作文本退化下降、anchor error降1.9477%，证明补偿有局部作用；但0/28 feasible、零switch、无checkpoint/PPL/lm_eval，单模型最后四层且缺逐group scale变化分布，仍只能作为机制负结果 | 停止scale clipping、group/bundle/seed/阈值扫描；只有先给出码率明确、非$O(N^2)$且显式text-restoring的跨Linear/低秩补偿构造，才允许下一方法实验，否则把assignment限定为task-adapted VQ | 8.0 | 5.8 | 6.0 | 5.6 | 8.3 | 5.5 |
