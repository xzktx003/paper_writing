...
> Convert to Wiley SPE LaTeX template before submission.
...
dependency tree. At the time of writing, `npm audit` reports 5 moderate,
11 high, and 1 critical findings. These primarily affect development
dependencies and build tooling rather than runtime behavior. We document
these findings and recommend triage before production deployment, but note
that zero-audit-findings is not a realistic goal for a JavaScript project
of this complexity.
...
See `manuscript/references.bib` for the complete bibliography.
