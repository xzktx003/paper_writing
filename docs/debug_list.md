...
- 2026-09-10：`tmp_stom` 这类包含大量嵌套 Git 仓库、`.omx` 日志和 Python 缓存的研究项目，打开编辑器时会先返回十万级文件树，前端长时间停留在“没有项目”并像是空白。修复为项目树在后端递归时直接跳过 `.git`、`.omx`、`node_modules`、虚拟环境、解释器缓存和测试缓存等运行时目录；论文、代码、报告和图表仍保留，避免把缓存规模传给浏览器。

- 2026-09-10：论文语言润色任务里写“保留引用键和 LaTeX 命令”时，路由把“命令”误当成需要执行 shell 工具，又把“保留已有引用”误当成需要新增文献证据，导致本应进入 Agent 的保守润色被错误升级为 Tools 并显示缺证据。修复为仅把“命令行/运行/执行”等动作识别为工具意图，并区分“保护已有引用”与“新增、检索、核对引用”；红绿测试锁定 `writing-polish`、Agent、无需 RAG 和可进入人工审阅流程。

- 2026-08-28：办公赛道参赛视频材料虽然已能展示实操，但旧说明偏技术术语，非技术评委难以快速理解；论文/Word/PPT 专题初版还暴露字幕描边不足和信息层级不清，视觉验收78→91。修复为登记 A/B 两支少于 3 分钟的白话视频、逐句字幕和逐字稿，把“4处→1任务、学术常见6处→1项目、重新整理2次→1次后复用”限定为示例动作变化，并通过字幕描边与信息层级修订转绿。
...
- 2026-07-24：点击“最终 PDF”原先在没有组件内缓存 URL 时直接调用完整编译，编辑正文时快速预览还会延迟触发全量编译，导致查看 PDF 与修改文本都可能产生非预期编译。修复为新增只读 `/api/compile/latest`，优先返回 `.compile/output/<main>.pdf`，兼容项目同目录 PDF；Final PDF 只查询并展示旧产物，所有重新编译均由显式按钮触发。Skill 下拉原先选择一项就关闭，修复为弹层内复选、显示待加入数量并一次批量提交。
- 2026-07-24：AI 流式回答原先把 Provider 的 15 秒安全超时绑定到整个响应生命周期，模型已经持续输出时仍会在 15 秒被底层 request 强制终止，前端显示半截内容和 `AI error: aborted`；SSE `event:error` 又被 Promise 当成成功完成，导致回退语义错误，半截回答也未落盘。修复为拆分响应头超时与 120 秒流空闲超时，持续 token 不受总时长限制；Provider 超时分别返回可理解错误，SSE 错误真正 reject 且不重复发送同一用户请求，中断的部分回答带错误状态持久化。缩短时钟红绿灯测试稳定复现旧 `ECONNRESET/ABORT_ERR` 并验证修复。
- 2026-07-24：AI 对话虽然允许在新建窗口输入任意文件路径，但实际工具仍只有 `read_chapter/list_chapters`，Chat 无文件工具，裸路径读取也只尝试 `sec/chapters`，导致模型看似只能查看章节目录。修复为 Chat/Agent/Tools 统一提供受管项目级 `list_project_files/read_project_file` 只读工具，递归覆盖根目录、Section、code 等位置并支持文本/PDF；新建对话默认把编辑器当前文件设为首要参考并先注入，同时提供全项目安全文件清单。隐藏目录、`.env`、密钥、符号链接、路径穿越、超大和不可读二进制文件继续阻止进入模型上下文。
- 2026-07-24：中央预览的“翻译”页独自使用旧非流式 `/api/ai/send`，模型连接失败时后端返回 HTTP 200 的 `{ error: true }`，前端又把 `reply: "AI error: Connection error."` 当译文展示；组件每次重新加载还会创建新的 `Preview Translate` 会话。修复为翻译复用正式 Chat 的 SSE 流式通道，逐 token 展示译文并把流式错误进入错误状态；请求携带 `ephemeralConversation`，后端在正常完成、模型失败或客户端中断的流生命周期末尾清理临时会话，前端也保留兜底删除，避免刷新或关闭页面后持续累积会话。
- 2026-07-24：翻译结果原先以 `white-space: pre-wrap` 纯文本显示，模型返回的 LaTeX 章节命令和公式无法按数学排版渲染；翻译状态又只有一个全局字符串，点击其他文件的翻译标签会清空前一个文件结果。修复为要求模型输出中文 GitHub Flavored Markdown，把行内/块级公式规范为 `$...$` / `$$...$$`，复用 MarkdownPreview 的 remark-math + KaTeX 渲染；结果按文件路径缓存，标签切换只展示缓存，显式“重新翻译”才替换当前文件译文。
- 2026-07-24：长章节翻译使用推理模型时约 46 秒后报 `AI error: Connection error.`。根因是 Provider 响应头默认仅等待 15 秒，OpenAI SDK 又默认重试两次，首 token 较慢时形成约 `15 秒 × 3` 的稳定失败。默认响应头等待提升到 60 秒，流建立后的 120 秒空闲上限保持不变，并将 SDK 自动重试显式限制为 1 次；回归锁定两阶段超时和有限重试，正式页面需用大章节实际翻译验收。
- 2026-07-24：对话消息虽然已使用 ReactMarkdown，但只启用了 GFM，模型返回的 `$...$` / `$$...$$` 仍显示为公式源码；直接复用全局 Markdown 公式颜色还会让暗色聊天气泡中的公式变成深色。修复为已完成消息与流式消息统一启用 remark-math + rehype-katex，不开启原始 HTML解析；聊天专用样式恢复主题正文色并允许块公式横向滚动，系统提示要求模型输出标准 Markdown 数学分隔符。
- 2026-07-24：CLI Task Agent 创建隔离快照时递归复制整个项目，Python 虚拟环境中的正常链接 `.venv/bin/python -> /usr/bin/python3` 被当成项目逃逸 symlink，导致任务在启动前失败。修复为复制、tree fingerprint、Diff 和 Accept 漂移检查统一排除 VCS、本地虚拟环境、依赖目录、缓存、编译产物与运行状态；排除名称若自身是 symlink 或快照范围内出现其他 symlink 仍严格拒绝。真实 `moe_prune/.venv` 场景用于正式任务创建验收。
- 2026-07-24：修复快照后，真实 Codex Task 继续暴露 Provider 假阳性：`codex login status` 因本地存在 API Key 返回 0，但远端 Responses WebSocket 实际返回 401。认证探针现区分“凭据存在”和“远端已验证”；Codex API Key 状态不再泄露掩码片段，也不再标记为 available，任务面板优先选择可被非交互检查确认的 Provider。
- 2026-07-24：Claude CLI Task 的固定安全参数使用 `--strict-mcp-config --mcp-config '{}'`，新版 Claude CLI 要求配置对象包含 `mcpServers`，导致所有任务在模型启动前报 `Invalid MCP configuration`。修复为传入 `{"mcpServers":{}}`，继续保持零 MCP server、无 Bash/网络权限；参数契约测试解析 JSON 并锁定空 server map。
- 2026-07-24：把服务器访问令牌改为 `1234` 时，根 `.env` 中的 `OPENPRISM_LLM_API_KEY` 和 Claude Key 也被错误覆盖为同一个 4 字符值，导致 `/models`、Chat Completions、Responses 与真实 Codex Task 全部返回 401；同时 Codex Task 仍默认访问旧的 `api.openai.com` 登录凭据，没有复用应用已经配置的兼容网关。修复为从本地保留配置恢复 LLM Key，保持 `OPENPRISM_API_TOKEN=1234` 独立不变；Codex Task/Chat 现在使用固定自定义 Responses Provider 复用应用 Base URL/Key，Key 仅进入专用子进程环境，readiness 直接验证配置 endpoint。正式 `moe_prune` 验收只使用 Codex：Task HTTP 202，最终 `waiting-review`、零 Diff、exit 0，随后 Reject 200，原工程未应用任何变化；普通只读 Provider invoke 也返回 HTTP 200、`CODEX_CHAT_OK` 和 exit 0。
- 2026-07-24：新版 Claude CLI 要求 `--print --output-format stream-json` 同时启用 `--verbose`，旧固定参数会在模型运行前退出。Task Agent 增加 `--verbose` 以满足流式 JSON 协议，不改变 Read/Edit/Write 文件权限、严格空 MCP 配置或无 Bash/网络边界。
- 2026-07-25：编译成功的 PDF 过去只主动写入隐藏的 `.compile/output/`，项目根目录 PDF 只是只读兼容回退，用户在工程文件中看不到本次最终产物。修复为 LaTeX 和 Markdown/Pandoc 共用原子持久化逻辑，同时更新内部稳定副本与项目根目录 `<主文件名>.pdf`，编译响应返回 `rootPdfPath`，最终 PDF 查询优先根目录；写入使用同目录临时文件后 rename，避免半截 PDF 和同名 symlink 跟随。正式 `moe_prune/main.tex` 编译 HTTP 200、exit 0，根目录与内部副本均为 1,093,231 字节 `%PDF-1.7` 且 SHA-256 完全一致，文件树显示 `main.pdf` 并继续隐藏 `.compile`。
- 2026-07-25：中央预览在已经存在 `compiledPdfUrl` 时，把 `Quick Preview` 和 `Final PDF` 两个标签都渲染成 `AuthenticatedPdf`，导致快速预览不再展示当前 LaTeX 源码的浏览器端近似排版。修复为 PDF 渲染条件严格限定 `previewTab === 'pdf'`；快速预览无论是否已有 PDF，都固定走 `RenderedPreviewPane` / `LatexPreview`，不读取 PDF、不隐式编译，最终 PDF 继续只读加载上一次显式编译产物。正式 `moe_prune` Playwright 验收：快速预览初始和从最终 PDF 切回后均显示 `.latex-preview-container`、PDF embed 为 0，切回过程新增 PDF 请求为 0；只有最终 PDF 标签显示 embed 并请求一次 `main.pdf`。
- 2026-07-25：Chat 流式等待此前只有固定 `Thinking...`，虽然 SSE 已发送 `tool_use` / `tool_result`，但 `useConversations` 未消费，用户无法判断模型在准备上下文、读取文件还是生成回答。现新增默认折叠的“工作过程”轨迹，覆盖阶段、RAG、工具开始/完成、生成、完成和失败；后端将原始工具输入/结果改为受限安全摘要，避免文件正文、修改全文和凭据泄露。该轨迹明确不是私有思维链，模型主动返回的 `<think>` 内容也改标为“模型提供的思考摘要”。
- 2026-07-25：左侧文件树的手动刷新按钮调用 `/api/projects/:id/files`，该接口返回带正文的文件集合而非 `{ path, type }` 文件树，目录会丢失；裸 `fetch` 在启用服务器访问令牌时还可能 401，因此按钮看似无效。现移除刷新按钮，改由 AppContext 使用认证 `/tree` 作为唯一目录同步源：文件操作、CLI Task Accept 和成功编译立即同步，前台页面每 2 秒及 focus/visibility 恢复时校验外部变化；同时通过认证文件 API 刷新已打开且未编辑的文本标签，外部编辑器、终端或 AI 工具写入后无需重新打开文件即可看到新内容。`dirty=true` 的本地草稿绝不自动覆盖，相同内容不更新 React 状态，临时网络失败保留旧状态。
- 2026-07-28：右下角悬浮终端入口固定压在 Assistant 对话区域上，窄窗口和长对话时会遮挡内容。现改为 Pointer Events 拖拽控件，位置限制在可视窗口与状态栏之上，并按项目写入既有布局本地存储；拖动结束不会误触打开终端，窗口尺寸变化时自动把按钮收回可视区域。
- 2026-07-28：对话输入框把附件、图片和发送三个按钮绝对定位在文本区域右下角，但只预留 48px 右内边距，导致行尾文字被按钮覆盖。现将操作按钮移到输入框下方独立操作行，文本区恢复完整可用宽度，长文本与多行输入不再被遮挡。
- 2026-07-28：Chat 流式请求前端没有保存或暴露当前 XHR 的取消入口，生成期间发送按钮又被禁用，因此用户无法中断 AI。现流式 API 接受 `AbortSignal`，停止按钮会 abort 当前 XHR，后端既有“连接断开即取消模型请求”逻辑随即生效；取消使用专用错误类型，明确禁止落入非流式 fallback，避免重复发送同一指令。
- 2026-07-28：受管项目的磁盘文件和 `/api/projects/:id/file` 已返回外部新内容，但浏览器标签仍可能显示旧值。根因有两层：历史 workspace 会持久化未保存草稿；CodeMirror 在接收 React 下发的外部新正文时又会触发 `docChanged`，把程序同步误判成用户编辑并重新标记 dirty。现为程序内容替换增加 CodeMirror transaction annotation，使用 `lastSyncedContent` 区分磁盘基线与用户输入；干净标签自动更新且保持未修改状态。workspace 状态升级到 version 2：修复前的 version 1 旧草稿不再压过当前磁盘文件，从而解决刷新后仍显示遗留 `-0.50` 的问题；version 2 开始才保留带磁盘基线的新草稿并在真实并发修改时显示冲突。真实 `Tab/full_qwen3_50.tex` 场景已覆盖：预置 version 1 的 `-0.50` 浏览器草稿且磁盘为 `0`，页面打开后直接显示 `0`；后续外部变化自动进入编辑器，冲突时保留新制草稿，选择外部版本后恢复干净状态，随后 Paper Writing 修改并保存也能从文件 API 读回。
- 2026-07-28：文件同步验收不能绑定 `full_qwen3_50.tex` 或某个数值替换。现扩展统一文本文件识别，覆盖 `.tex/.md/.bib`、JSON/CSV/TSV、常用代码与 shell、Notebook、样式/配置文件以及 Dockerfile/Makefile/README 等无扩展名文本；生产源码中不存在目标文件名或 `-0.50 → 0` 特判。隔离 Chromium 另外以运行时随机内容连续修改 `notes.md`、`analysis.py`、`results.json`，三个已打开标签都在轮询窗口内读取到磁盘新值并保持 clean。
- 2026-08-27：项目首页已将默认标题收敛为“全部项目”并将物理目录文案明确为“工程文件夹”，但移动端和项目生命周期 E2E 仍断言旧文案，导致完整门禁出现 3 个非功能失败。已把测试更新为当前可见的无障碍标题和项目身份文案，保持对移动可操作性、稳定 ID 与真实物理目录的原有验收语义。
- 2026-08-27：仓库曾追踪 `app/apps/frontend/dist/index.html`，但其引用的带哈希 JS/CSS 资产按 `.gitignore` 不会提交，每次构建都会让远程仓库中的 HTML 指向不存在的 bundle。现将该生成 HTML 一并从版本控制中移除，保留本地构建产物；部署继续以 `npm run build` 生成完整、相互匹配的 `dist/`。
- 2026-08-27：办公赛道审核初版在读取状态时会刷新 `updatedAt`，重复导出又可能因来源路径与固定 `submission/*` 产物碰撞而改变审核结论和哈希。修复为读取保留持久时间戳、拒绝来源指向生成文件、以状态时间生成 manifest，并用同状态重复导出哈希一致回归锁定单向证据链。
- 2026-08-27：办公赛道审核初版会把负向提效截断为 0、把量化数据不完整误判成“只有创意”、只因预先填写答辩问题就标记问答已准备，并在演示文件实际缺失时保留总建议分。修复为保留负值且实测无改善时提效模块为 0，有可用演示时不对缺量化设置统一上限，缺答辩记录始终待现场评分，演示不可读时总分为 `null`，矛盾证据统一转人工复核。
- 2026-08-27：全局终端悬浮按钮默认停在右下角时会遮挡办公交付面板的“保存当前记录”，桌面和 390×844 移动端都可能阻断关键操作。默认位置上移到粘性操作栏之外，仍保留拖动与项目级位置记忆；浏览器回归直接比较两个按钮的矩形区域，锁定零重叠。
- 2026-08-27：办公赛道参赛手册初版漏列 `initial-score-guide.md`、`finals-pack.md` 和 `finals-score-guide.md`，README 还用了容易被误读为真实路径的 `submission/M01-M06`。现以导出器的 12 个固定产物为唯一口径，补齐初赛、决赛和 manifest 清单，并改用可核对的 `M01-*.md` 到 `M06-*.md` 表达。
- 2026-08-27：OfficeCLI 适配器初稿按概念操作名推测了不存在的上游参数，且误以为上游提供 diff。通过 GitHub v1.0.145 release 二进制与 `--help` 实测后，映射改为真实的 `dump/create/batch/merge/view/validate`，每次执行设置 `OFFICECLI_SKIP_UPDATE=1`；差异明确由应用内置 OOXML 文本抽取完成。
- 2026-08-27：真实 OfficeCLI template merge 生成的 DOCX 使用 `<w:t xml:space="preserve">`，内置解析器最初只处理字符串型 `t`，导致合法文本被漏读。`collectText` 现同时处理带属性对象的 `#text`，并用真实合并产物的结构回归锁定。
- 2026-08-27：中文证据图遇到同一段“试点已批准，但演练未减少耗时”时，段内否定词会把批准主张也判成冲突。冲突分类现要求否定与主张共享的动作词在局部前置上下文绑定，证据缺口也只与证据类主张关联；中英文单测和六阶段 E2E 同时锁定支持/冲突并存。
- 2026-08-27：V2 审阅/审批 API 初稿允许在 triggered 状态提前写评论或批准事件，界面还把本地 published 状态描述成文件已发布到项目目录。服务现强制评论、回复、建议决策和审批事件仅在 review 写入；界面明确 published 只是本地工作流状态，参赛包仍需在交付阶段单独人工确认导出。
...
- 2026-08-28：旧版办公赛道主演示按阶段各截一张图，处理阶段还把未配置 OfficeCLI 的 `unavailable` 当作主要结果；DOCX、会议、证据图、度量和审核虽有实现，却没有把输入与成功详情完整入镜。修复为录制前强制校验绝对且可执行的 `OFFICECLI_PATH`，逐项断言 OfficeCLI `ok / validate`、解析详情、会议决定/待办、support/conflict/missing、人工审批、designed 边界、94/100 low 与导出文件；六阶段拆为 19 张截图并生成 `coverage.json`，任一功能失败即中止，不再产出表面完整的参赛视频。
- 2026-08-28：第二版录制首次构造的“长期稳定提效”反证没有在共享动作词附近直接否定“证明”，关系分类器按高重叠正确返回 support，导致演示契约停在 conflict 断言。受控反证改为“未证明稳定提效”，并先用服务函数独立验证 conflict 后重录；视觉验收另发现 designed 边界和 94/100 总分滚出视口，改用关键结果节点作为取景锚点后复录通过。
- 2026-08-28：办公主流程 E2E 仍断言旧的英文内部状态 `sufficient`，而界面已经按中文用户语义显示“字段完整”，导致功能真实计算出 38 分钟后测试仍误报失败。测试改为断言当前可见文案“字段完整”和计算值 38，继续锁定同一成功行为。
- 2026-08-28：重制讲解视频首轮把转换后的中文字体按外部名称 `Noto Sans SC` 交给 libass，字体真实内部名称却是 `Noto Sans SC Thin`，导致烧录字幕出现方框；修正字体族后，SRT 又按 libass 内部画布放大，34 号字幕遮挡了实操。生成器现按标点把字幕切为最多 23 字，使用真实字体族，并把实测字号/描边/底边距校准为 14/1.3/10；三轮抽帧从 66、68 最终提升到 94 分，字幕稳定落在独立底部安全区。
- 2026-08-28：讲解视频契约测试最初用 `process.cwd()` 推导仓库位置，在 `app/` 内运行通过、从根级组合命令运行却读取错误目录。测试现基于 `import.meta.url` 固定解析仓库根，根级与 app 级运行语义一致。
- 2026-08-28：输入/输出详解版成片首次沿用旧度量/交付裁剪边界，度量右栏末尾曾短暂出现交付页；第一次前移又把空白过渡帧冻结到旁白末尾。通过逐帧检查原录屏 57.5—59.0 秒，最终将度量终点锁在 58.0 秒的 `measurementStatus=designed` 成功画面，交付从 61.5 秒稳定页面开始，并用契约测试锁定两个边界。
- 2026-08-28：成片生成器默认使用 PATH 中的 `edge-tts`，却先对字面值执行 `fs.access('edge-tts')`，导致命令已经安装仍可能因当前目录没有同名文件而误报 ENOENT。移除错误文件检查，保留真正执行 `edge-tts --version` 的可用性探针；显式 `EDGE_TTS_PATH` 仍可覆盖命令位置。
