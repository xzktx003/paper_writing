...
- 基线提交：`18fceb2 Make the office demo explain why each operation matters`；本文件记录其上的输入/输出/人工对照增强候选工作树
...
| 全量单元测试 | 523/523 通过 | 96 个测试文件，包含双视频白话讲解、提交包、演示覆盖与办公服务回归 |
...
| 双视频讲解契约 | 3/3 通过 | 两支独立视频、字幕、逐字稿、封面、10 张讲解页齐备；旁白不依赖命令名、英文状态或校验算法等术语 |
| 演示脚本语法 | 6/6 通过 | 录像、双视频成片剪辑、两份视频规格、PDF、manifest 通过 `node --check` |
...
| A 通用办公提交成片 | H.264/AAC MP4 7,770,180 bytes；163.040 秒 | 1920×1080、yuv420p、48 kHz 双声道；6 张动画讲解页、6 段实操、中文讲解和 77 条逐句字幕；完整 10 张讲解页另行保留 |
| B 论文/Word/PPT 专题成片 | H.264/AAC MP4 6,220,635 bytes；148.174 秒 | 1920×1080、yuv420p、48 kHz 双声道；10 张动画讲解页、中文讲解和 64 条逐句字幕 |
| 功能覆盖契约 | 5/5 通过 | 六阶段 success；OfficeCLI `ok`；19 张截图；每阶段至少两张；每张超过 50 KB |
| 完整包清单 | 101 个文件 | 必交项无缺失，逐文件字节数与 SHA-256 由契约测试复算 |
