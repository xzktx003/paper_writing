...
| 全量单元测试 | 522/522 通过 | 96 个测试文件，包含讲解成片、提交包、演示覆盖与办公服务回归 |
...
| 参赛提交成片 | H.264/AAC MP4 7,058,716 bytes；165.791 秒 | 1920×1080、yuv420p、48 kHz 双声道；10 张 PPT、6 段实操、中文讲解和 57 条逐句字幕 |
| 功能覆盖契约 | 5/5 通过 | 六阶段 success；OfficeCLI `ok`；19 张截图；每阶段至少两张；每张超过 50 KB |
| 完整包清单 | 86 个文件 | 必交项无缺失，逐文件字节数与 SHA-256 由契约测试复算 |
