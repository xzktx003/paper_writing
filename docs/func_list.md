...
## Paper Writer 论文写作助手（2026-09-10）

- 正式 `/projects` React 工作台右侧新增“写作”入口，直接复用 `/api/projects/:id/writing-workbench/*` 聚合能力，不再要求用户打开默认关闭的 Legacy 原型。
- 写作助手接收自然语言论文任务、当前打开文件和可选 RAG 检索词；输出任务路由、推荐模式及理由、论文写作流程、生产准备度、推荐 Skill 的输入/输出、证据状态、命中片段和下一步操作队列。
- 常用任务由后端 `taskStarters` 驱动，覆盖论文规划、Introduction、Related Work、Method、Results/Discussion、Abstract、Conclusion、语言润色和 AI 草稿证据审查；点击只分析并填入任务，不自动调用模型。
- “润色当前文件”固定要求保留公式、已有引用键、数字和 LaTeX 命令，不新增事实、不直接覆盖正文；任务进入 `writing-polish` + Agent，点击“带到 AI 对话”后只新建/切换到匹配模式并填入安全提示词，最终发送和 Diff 采纳仍由用户明确操作。
- “审查 AI 草稿”调用 `review-answer`，展示可采纳/需修改/拒绝结论与阻塞项，用同一任务、目标文件、检索词和证据指纹检查来源编号、证据越界与上下文缺口。
- 顶部十个功能标签改为可横向滚动的可读宽度，避免在 380px 助手栏内被平均压缩成难以点击的小标签。
- 新增 UI 契约、保守润色路由和隔离 Chromium 端到端测试；端到端验收覆盖打开 `main.tex`、一键润色、选择 `writing-polish`/Agent、把提示词带入对话且不自动发送或写文件。
...
- RAG 面板采用“导入即自动重建索引”的单一语义；后端同时提供受测的 `/rag/index` 与 `/rag/search` 契约，搜索结果保留来源路径与证据片段。项目普通文件树默认隐藏 `.openprism`、`.compile`、`research_corpus` 以及 `.git`、`.omx`、`node_modules`、虚拟环境、Python/测试缓存等运行时目录；证据资料统一从 RAG 面板管理，避免大型研究项目把缓存条目传给浏览器。
