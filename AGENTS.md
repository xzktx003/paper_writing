...
## OpenPrism Office 交付约定

- 正式产品入口仍为 `/projects`；办公材料交付通过工作台右侧“交付”面板完成，不得把 Legacy 静态原型重新设为默认入口。
- 办公交付状态位于项目内 `.openprism/office-track.json`，用户可见产物位于项目内 `submission/`；两者都必须通过 managed `projectId`、路径校验和现有鉴权访问。
- 准备建议分不是官方评分。没有真实基线、样本、测量周期、人工复核或落地记录时，UI 与导出必须保留“待补充”，严禁生成虚构提效或获奖承诺。
- 新增或修改办公赛道规则时，必须同步 `docs/office_track_rule_matrix.md`、直接相关红绿灯测试和 `docs/competition/` 的操作材料。
