...
The catalog now targets AI/ML research and CCF-A, ICLR, ICML, and NeurIPS submissions. All MedSci medical Skills were removed. It retains 74 general-purpose AlterLab Skills and adds SNL's top-tier paper-writing workflow, AI4S, the Chinese ResearchPilot pipeline, and non-medical entries from SkillsBot's Scientific Academic category. Complete GitHub packages keep their references, scripts, assets, and evals under `app/apps/backend/skill-resources/`.
...
[`open-source-skills.manifest.json`](app/apps/backend/skills/open-source-skills.manifest.json) records repositories, exact commits, filters, and generated files. GitHub licenses and notices remain in their resource trees. SkillsBot entries without a published license are explicitly marked for license verification. AlterLab's four derived core workflow Skills retain the upstream CC-BY-NC 4.0 non-commercial restriction.
