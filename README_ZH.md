...
受管项目的文件树会自动同步真实目录，不需要手动刷新按钮。应用内文件操作、CLI Task 接受和 PDF 编译完成会立即触发同步；项目在前台打开时也会低频检查由终端、外部编辑器或其他工具新增、删除、重命名或修改的文件。已打开且没有本地未保存修改的文本标签会自动刷新为外部内容；Paper Writing 内保存的修改也会写回同一磁盘文件。若外部内容和未保存草稿同时变化，界面会保留草稿并提示重新加载外部版本，不会静默覆盖任一侧内容。
