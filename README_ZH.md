...
当前目录采用面向 AI/ML、CCF-A 与 ICLR/ICML/NeurIPS 投稿的科研 Skill 配置。MedSci 医学技能已全部移除；保留 74 个通用 AlterLab Skill，并加入 SNL 顶会论文写作、AI4S、ResearchPilot 中文科研流程，以及 SkillsBot“科研学术”分类中的非医学条目。它们与原有 Skill 一样以 YAML 加载，完整 GitHub 包的 references、scripts、assets 和 evals 位于 `app/apps/backend/skill-resources/`。
...
同步清单记录了来源仓库、精确 commit、过滤数量和文件列表：[`open-source-skills.manifest.json`](app/apps/backend/skills/open-source-skills.manifest.json)。GitHub 来源保留许可证和第三方声明；SkillsBot 未公开标注许可证的条目会在 YAML 中明确标记“请核对原作者许可”。AlterLab 的四个 core Skill 仍带 CC-BY-NC 4.0 非商业限制。
