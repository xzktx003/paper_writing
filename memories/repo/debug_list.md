...
- 2026-09-10：大型论文/研究项目打开时若包含嵌套 `.git`、`.omx`、`node_modules`、虚拟环境或 Python 测试缓存，项目树接口会递归出十万级条目，前端初始化期间显示空项目。`listFilesRecursive` 现在支持 `skipDirectories`，正式项目树使用运行时目录黑名单在递归阶段剪枝；可见论文与代码文件不受影响，回归测试覆盖嵌套 `.git` 和 `__pycache__`。

- 2026-09-10：论文润色提示中的“保留引用键/LaTeX 命令”不能触发 Tools 或缺证据门禁；路由现区分受保护语法与执行动作、区分保留已有引用与新增/核对引用，保守润色固定进入 `writing-polish` + Agent，并继续要求 Diff 人工确认。

- 2026-08-28：办公赛道视频说明偏技术化，且论文/Word/PPT 白话专题初版存在字幕描边弱、信息层级不清的问题，视觉验收78→91。修复为在参赛文档中登记 A 通用办公白话动画+六步实操、B 论文/Word/PPT 白话专题两支少于 3 分钟视频，明确逐句字幕/逐字稿路径，并把示例量化限定为动作结构变化；真实节省百分比仍待试点。
...
- 2026-08-27：办公赛道审核状态读取必须保留持久 `updatedAt`；来源材料不得指向导出器固定覆盖的 `submission/*` 文件。否则重复导出会改变材料可读性、建议分和哈希，形成生成物自证循环。
- 2026-08-27：办公赛道规则边界必须保留负提效、把实测无改善归零、避免对“有实操但量化不完整”统一限分、在无答辩记录时保持“待现场评分”，并在演示不可读或矛盾证据出现时取消总建议分/转人工复核。
- 2026-08-27：全局终端悬浮按钮的默认位置必须避开底部粘性操作栏；移动和桌面浏览器测试应以元素矩形零重叠验证，不能只验证按钮存在。
- 2026-08-27：参赛文档中的导出文件名必须与 `officeTrackService` 的固定产物清单逐项一致；不得用 `submission/M01-M06` 这种像真实路径却无法访问的缩写，并要同时列明初赛建议分、空白评分表、决赛准备与 manifest。
- 2026-08-27：OfficeCLI 适配必须以已固定版本的真实 `--help` 和二进制 smoke 为准，不从概念操作名推测命令。v1.0.145 使用 `dump/create/batch/merge/view/validate` 且没有 diff；运行时禁用自动更新，diff 留在内置 OOXML 抽取层。
- 2026-08-27：OOXML `<w:t>` 可能因 `xml:space="preserve"` 解析为含 `#text` 的对象；文本收集必须同时覆盖字符串和属性对象，否则 OfficeCLI/Word 生成的合法 DOCX 会静默漏字。
- 2026-08-27：证据冲突不能按整段是否出现否定词判断。否定必须与主张共享动作词的局部前置上下文绑定；一段中可同时支持“已批准”并冲突“已减少耗时”。
- 2026-08-27：办公协同写操作只能发生在 review；approved/published 必须由 human approval event 解锁。本地 published 是审计状态，不等于文件已复制到外部系统或交付目录。
...
- 2026-08-28：FFmpeg/libass 不能在当前环境直接使用前端 WOFF2 生成中文烧录字幕；参赛成片构建应把随包 Noto Sans SC 临时转换为 TTF 并通过 `fontsdir` 显式注入。白色办公界面上的字幕必须有深色底板，构建后删除临时字体并抽帧验证。
- 2026-08-28：参赛主演示必须以成功状态断言驱动，不得用“外部能力未配置”的失败画面代表主要功能。当前录制器要求真实 OfficeCLI `ok / validate`，并以 19 张截图覆盖六阶段的输入、执行、结果与风险边界；任一断言失败立即终止并拒绝生成覆盖率证据。
- 2026-08-28：视觉证据不能依赖覆盖字幕替代界面本身。长列表截图应把 designed 测量边界、准备建议总分/置信度、风险和导出文件分别锚定取景，单个阶段允许且应使用多张截图。
- 2026-08-28：Office 度量成功态的中文可见文案是“字段完整”，不是内部英文值 `sufficient`；浏览器测试应锁定当前用户可见语义与计算结果，避免界面翻译后产生假回归。
- 2026-08-28：FFmpeg/libass 烧录 SRT 时必须使用转换后字体的真实内部族名 `Noto Sans SC Thin`；SRT 默认画布还会让直觉字号显著放大，最终应以 1920×1080 抽帧验收而不是只看参数。本仓库讲解视频的稳定参数为每条最多 23 字、FontSize 14、Outline 1.3、MarginV 10，并保留独立 120px 字幕安全区。
- 2026-08-28：仓库测试不得用 `process.cwd()` 假设从某个 package 目录启动；需要读取跨目录固定资产时，以测试文件的 `import.meta.url` 解析仓库根，确保根级和子包级命令一致。
- 2026-08-28：参赛成片的功能章节边界必须按原录屏稳定结果帧验收，不能只照截图时间戳粗切。当前度量段固定使用原录屏 53.4—58.0 秒并冻结 `designed` 成功记录，交付段从 61.5 秒开始；契约测试锁定这两个值，防止章节错位或冻结空白过渡帧。
- 2026-08-28：PATH 命令不能用 `fs.access('command-name')` 判断是否存在；该调用只检查当前目录文件。参赛视频生成器通过真正执行 `edge-tts --version` 验证命令，只有用户提供显式文件路径时才需要文件级检查。
